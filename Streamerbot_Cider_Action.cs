    ...
    public bool Execute()
    {
        ...
        try
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                {
                    client.DefaultRequestHeaders.Add("CIDER API TOKEN HERE", apiToken);
                }
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        string message = $"/me {name} by {artist}";
                    ...
                    {
                        CPH.SendMessage("/me No song is currently playing or Cider is idle.");
                    }
                }
                else if ((int)response.StatusCode == 401)
                {
                    CPH.SendMessage("/me Cider API Authentication failed. Please check your API Token in the script.");
                }
                else
                {
                    CPH.SendMessage("/me Could not reach Cider. Ensure it's open and the Web API is enabled in Settings.");
                }
            }
        }
        catch (Exception ex)
        {
            ...
            CPH.SendMessage("/me Error connecting to Cider. Is it running?");
        }
